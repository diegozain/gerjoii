...
% ------------------------------------------------------------------------------
% shape 
% 1. for finding receivers on the trunk of the tree & 
% 2. as mask for the gradient in update methods.
tmp_=load('../../image2mat/nature-synth/mat-file/shape.mat');
tmp_=tmp_.shape;
parame_.shape = tmp_;
% ------------------------------------------------------------------------------
% truth
...
% -
% permittivity: circle in circle
% conductivity: circle in circle
% load('../../image2mat/nature-synth/initial-guess/sigm_init_.mat');
% load('../../image2mat/nature-synth/initial-guess/epsi_init_.mat');
% -- w --
...
% -----------------------
gerjoii_.w.obj_FNCs    = {'L2','phase_L2'}; % L2, env_L2, phase_L2
gerjoii_.w.g_e_weights = [1,p_inv(5)];
gerjoii_.w.g_s_weights = [1,p_inv(6)];
% -----------------------
% save?
% -----------------------
...
% step size
% ..............
gerjoii_.w.k_e = [1e-3 1e+1]; % 1e-8 1e+2. 1e-5 1e-1
gerjoii_.w.k_s = [1e-3 1e+2]; % 1e-7 1e+5
...
% -----------------------
% permittivity
eps_max_ = 7; % max(parame_.natu.epsilon_w(:));
