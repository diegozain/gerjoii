...
% Set the arguments to pass to Slurm
% At a minimum we suggest setting a time limit that matches the main job using the -t argument
% If using '--mem-per-cpu' to set the amount of memory you need for each task then you need to also set that arguement in the main job.
c.SubmitArguments='-p geop -t 0-11:59:00 --mem-per-cpu=25G';
parpool(c,poolsize);
% parpool( poolsize );
fprintf('\n\n    ------------------------------\n');
...
% -----------------------
gerjoii_.wdc.tol_iter  = 1; % tol_iter;
