...
for iruns=1:nruns
  iruns_=runs(iruns);
  load(strcat('all/','p_inv',num2str(iruns_),'.mat'));
  load(strcat('all/','as',num2str(iruns_),'.mat'));
  % ----------------------------------------------------------------------------
  as(:,:,1)=[];
...
E_(1:ni)        = E_(1:ni)+Ew_;
% ------------------------------------------------------------------------------
dlmwrite('ml/data/EE.txt',EE,'delimiter',' ','precision',20);
dlmwrite('ml/data/P.txt',P,'delimiter',' ','precision',20);
dlmwrite('ml/data/E_.txt',E_,'delimiter',' ','precision',20);
% ------------------------------------------------------------------------------
figure;
hold on
for iruns=1:nruns
