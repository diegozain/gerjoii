...
model = tf.keras.models.Sequential([
    ...
    # tf.keras.layers.InputLayer(input_shape=(pixSize,pixSize,3)),
    #
    # start the image processing part
    ...
    # 
    tf.keras.layers.Conv2D(me*(me-1), (nfy,nfx), activation='relu'),
    tf.keras.layers.MaxPooling2D(2,2),
    # 
    tf.keras.layers.Conv2D(me*(me-1), (nfy,nfx), activation='relu'),
    tf.keras.layers.MaxPooling2D(2,2),
    #
    # we're done with image processing, now it's in vector mode
    tf.keras.layers.Flatten(),
    #
    # rnn to get the permittivity sensitivity
    # tf.keras.layers.Reshape( (46464,1) ),
    # tf.keras.layers.SimpleRNN( me*(me-1)*ml ),
    tf.keras.layers.SimpleRNN( me*(me-1)*ml , input_shape=[None,46464]),
    #
    # add some dense layers to fix small details
