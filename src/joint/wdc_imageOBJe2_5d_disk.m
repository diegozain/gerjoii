function [parame_,gerjoii_] = wdc_imageOBJe2_5d_disk(geome_,parame_,finite_,gerjoii_)
% diego domenzain.
% @ Mines, summer 2020
% ------------------------------------------------------------------------------
...
while (E_>tol_error & iter<tol_iter)
  ...
  if gerjoii_.w.g_s_weights(2) == 0
    gerjoii_ = w_update_s_disk_(geome_,parame_,finite_,gerjoii_);
  else
    gerjoii_ = w_updateOBJ_s_disk_(geome_,parame_,finite_,gerjoii_);
  end
  % - smooth out the parts that got botched in the stack :(
  ax = gerjoii_.w.regu.ax*2; 
  az = gerjoii_.w.regu.az*2;
  nx_pad = uint32(geome_.m);
  nz_pad = uint32(geome_.m*0.5);
  gerjoii_.w.dsigma = image_gaussian_pad(gerjoii_.w.dsigma,ax,az,'LOW_PASS',nx_pad,nz_pad);
  % -
  ...
  if gerjoii_.w.g_e_weights(2) == 0
    gerjoii_ = w_update_e_disk_(geome_,parame_,finite_,gerjoii_);
  else
    gerjoii_ = w_updateOBJ_e_disk_(geome_,parame_,finite_,gerjoii_);
  end
  % - smooth out the parts that got botched in the stack :(
  ax = gerjoii_.w.regu.ax*2; 
  az = gerjoii_.w.regu.az*2;
  nx_pad = uint32(geome_.m);
  nz_pad = uint32(geome_.m*0.5);
  gerjoii_.w.depsilon = image_gaussian_pad(gerjoii_.w.depsilon,ax,az,'LOW_PASS',nx_pad,nz_pad);
  % -
