function w_adjoint_(geome_,finite_,gerjoii_,parallel_memory_w,is)  
% diego domenzain
% boise state university, 2018.
% ------------------------------------------------------------------------------
% computes and saves the adjoint wavefield in case you want to see it.
% ------------------------------------------------------------------------------
e_w = gerjoii_.w.e_2d;                   
Mw = gerjoii_.w.M;
dt = geome_.w.dt;
...
% path to save adjoint wavefield,
% a_folder = parallel_memory/w/source#/a/
% parallel_memory_w = '/path/to/disk/parallel_memory/w/'
a_folder = strcat(parallel_memory_w,'source',num2str(is),'/a/');
% access chunks in reverse
chunks = flip(gerjoii_.w.chunks,2);
% initialize first chunk
a_w = zeros(numel(domain_i(:)),numel(domain_j(:)),chunks(1),'single');
it_=1;
i_chunk=1;
% % adjoint (for Kurzmann's filter)
% a_w_ = zeros(numel(domain_i(:)),numel(domain_j(:)));
% tic;
for it = 0:nt-1
% --------
% updates
...
if ( it_ <= chunks(i_chunk) )
  % save adjoint wavefield frame 
  a_w(:,:,it_) = Ez(domain_i(:),domain_j(:));
  it_ = it_+1;
else
  % save chunk
  name = strcat(a_folder,'a',num2str( i_chunk ),'.mat');
  ...
  a_w = zeros(numel(domain_i(:)),numel(domain_j(:)),chunks(i_chunk));
