...
k_ = -bwd_fwd * dc_boundstep2(parame_.dc.sigma,bwd_fwd*gerjoii_.dc.g,...
                  gerjoii_.dc.regu.sig_max_ ,gerjoii_.dc.regu.sig_min_,...
                  gerjoii_.dc.k_s);
k_ = gerjoii_.dc.kprct_*k_;
% right side of parabola
k__ = dc_boundstep2(parame_.dc.sigma,-gerjoii_.dc.g,...
                  gerjoii_.dc.regu.sig_max_ ,gerjoii_.dc.regu.sig_min_,...
                  gerjoii_.dc.k_s);
...
% get obj values
% Edc is with k=0
k_  = linspace(k_,k__,4).'; % 3
Edc_=zeros(numel(k_),1);
for i_=1:numel(k_)
  % fwd & obj fnc
  Edc_(i_) = dc_objs2_5d(geome_,parame_,finite_,gerjoii_,k_(i_));
end
% ------------------------------------------------------------------------------
% bundle together
Edc = [Edc ; Edc_];
...
if step_<0
  ...
  fprintf('         polyval(pdc,linspace(min(k),max(k),1e+3)))\n\n');
  % ----------------------------------------------------------------------------
  [~,Edc_i] = min(Edc);
  if k(Edc_i)>0
    step_=k(Edc_i);
    fprintf('\n    fixed even though step sigdc is negative = %2.2d\n',step_);
  else
    step_ = 0;
  end
end
