...
% ------------------------------------------------------------------------------
% WARNING:
% run after datavis_w.m, field_w.m & swvlets_w.m
% ------------------------------------------------------------------------------
...
% ------------------------------------------------------------------------------
% unclip
prompt = '\n\n    Do you want to unclip the data (y or n):  ';
unclip = input(prompt,'s');
% ------------------------------------------------------------------------------
% constants
cf = field_.w.cf;
nl = field_.w.nl;
...
for is=1:ns;
  ...
  if strcmp(unclip,'y')
    ...
    d = w_unclip_(d,dt,f_,f__,nf,it_ini,it_end);
  end
  % ----------------------------------------------------------------------------
  % muting
...
%   size of wave cube
% ------------------------------------------------------------------------------
fprintf('\n\n   wave cube will be of ** singles ** size (no pml, no air) %1d [Gb]\n\n',...
          numel(x)*numel(z)*numel(t)*4*1e-9);
% ------------------------------------------------------------------------------
%   wavelets
% ------------------------------------------------------------------------------
...
% must choose existing project:
ls('../raw/');
fprintf('\n now we check this was done right\n')
prompt = '\n    Tell me what project you want (the one you just did):  ';
project_name = input(prompt,'s');
data_path__ = strcat('../raw/',project_name,'/w-data/','data-mat-fwi/');
data_name_  = 'line';
% ------------------------------------------------------------------------------
